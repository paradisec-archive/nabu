...
export class AppStack extends cdk.Stack {
  constructor(
    ...
    const searchTaskDefinition = new ecs.Ec2TaskDefinition(
      this,
    ...
    const searchContainer = searchTaskDefinition.addContainer(
      "SearchContainer",
      {
        memoryLimitMiB: 1536,
    ...
    const proxyistTaskDefinition = new ecs.Ec2TaskDefinition(
      this,
    ...
    const viewerTaskDefinition = new ecs.Ec2TaskDefinition(
      this,
    ...
    const commonAppImageOptions: ecs.ContainerDefinitionOptions = {
      ...
      environment: {
        ...
        SENTRY_DSN:
          "https://aa8f28b06df84f358949b927e85a924e@o4504801902985216.ingest.sentry.io/4504801910980608",
        DOI_PREFIX: "10.26278",
        DATACITE_BASE_URL: "https://mds.datacite.org",
        AWS_REGION: region,
      },
      secrets: {
        SECRET_KEY_BASE: ecs.Secret.fromSecretsManager(
          appSecrets,
          "secret_key_base",
        ),
        NABU_DATABASE_PASSWORD: ecs.Secret.fromSecretsManager(
          db.secret!,
          "password",
        ),
        NABU_DATABASE_HOSTNAME: ecs.Secret.fromSecretsManager(
          db.secret!,
          "host",
        ),
        RECAPTCHA_SITE_KEY: ecs.Secret.fromSecretsManager(
          appSecrets,
          "recaptcha_site_key",
        ),
        RECAPTCHA_SECRET_KEY: ecs.Secret.fromSecretsManager(
          appSecrets,
          "recaptcha_secret_key",
        ),
        SENTRY_API_TOKEN: ecs.Secret.fromSecretsManager(
          appSecrets,
          "sentry_api_token",
        ),
        DATACITE_USER: ecs.Secret.fromSecretsManager(
          appSecrets,
          "datacite_user",
        ),
        DATACITE_PASS: ecs.Secret.fromSecretsManager(
          appSecrets,
          "datacite_pass",
        ),
      },
    ...
    loadBalancer.connections.allowTo(
      autoScalingGroup,
      ec2.Port.allTcp(),
      "Allow from LB to ECS service",
    );
    ...
    const jobsTaskDefinition = new ecs.Ec2TaskDefinition(
      this,
      "JobsTaskDefinition",
    );
    ...
    if (env === "prod") {
      const cronTaskDefinition = new ecs.Ec2TaskDefinition(
        this,
        "CronTaskDefinition",
      );
    ...
    const sslListener = elbv2.ApplicationListener.fromLookup(
      this,
      "AlbSslListener",
      {
        loadBalancerArn: ssm.StringParameter.valueFromLookup(
