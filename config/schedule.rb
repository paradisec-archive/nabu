...
job_type :nabu_rake,  "cd :path && if [ ! -f tmp/pids/disable_cron ]; then :environment_variable=:environment flock --nonblock tmp/locks/:lock.lock :bundle_command rake :task DRY_RUN=true --silent :output; fi"
