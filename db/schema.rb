...
ActiveRecord::Schema[8.0].define(version: 2025_10_15_035451) do
  ...
  create_view "entities", sql_definition: <<-SQL
      select `collections`.`id` AS `entity_id`,'Collection' AS `entity_type`,`collections`.`identifier` AS `identifier`,`collections`.`title` AS `title`,count(distinct `collection_items`.`id`) AS `items_count`,count(distinct `collection_essences`.`id`) AS `essences_count`,`collections`.`private` AS `private`,group_concat(distinct `collection_essences`.`mimetype` order by `collection_essences`.`mimetype` ASC separator ',') AS `media_types` from ((`collections` left join `items` `collection_items` on((`collections`.`id` = `collection_items`.`collection_id`))) left join `essences` `collection_essences` on((`collection_items`.`id` = `collection_essences`.`item_id`))) group by `collections`.`id` union select `items`.`id` AS `entity_id`,'Item' AS `entity_type`,`item_collections`.`identifier` AS `identifier`,`item_collections`.`title` AS `title`,0 AS `items_count`,count(distinct `item_essences`.`id`) AS `essences_count`,`items`.`private` AS `private`,group_concat(distinct `item_essences`.`mimetype` order by `item_essences`.`mimetype` ASC separator ',') AS `media_types` from ((`items` left join `collections` `item_collections` on((`items`.`collection_id` = `item_collections`.`id`))) left join `essences` `item_essences` on((`items`.`id` = `item_essences`.`item_id`))) group by `items`.`id`
  SQL
end
