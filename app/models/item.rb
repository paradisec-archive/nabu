...
class Item < ApplicationRecord
  ...
  def search_data
    data = {
      ...
      data_types: data_types.map(&:name).uniq,
      filename: essences.map(&:filename).uniq,
      mimetype: essences.map(&:mimetype).uniq,
      ...
      data_category_ids: data_categories.map(&:id).uniq,
      data_type_ids: data_types.map(&:id).uniq,
      subject_language_ids: subject_languages.map(&:id).uniq,
      collection_id:,
      collector_id:,
      operator_id:,
