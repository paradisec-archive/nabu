$(function () {
  $('#add-university').on('click', function() {
    const input = $('<div id="add-university-form"><input name="university[name]" id="university_name"><button>Add</button></div>');
    $(this).replaceWith(input);

    $('#add-university-form button').on('click', function () {
      const value = $('#university_name').val();
      $.post('/universities', 'university[name]=' + value, function (data) {
