...
class CollectionsController < ApplicationController
  ...
  # by load_and_authorize_resource. Re-add auth for everything except those GeoJSON feeds; every
  # other action - including HTML index/show and search - stays login-gated. (Conditional
  # skip_before_action is unreliable here, so we skip wholesale and re-add with the condition.)
  skip_before_action :authenticate_user!
  before_action :authenticate_user!, unless: -> { request.format.geo_json? && action_name.in?(%w[index show]) }
