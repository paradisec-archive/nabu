...
class CollectionProvider < OAI::Provider::Base
  ...
  source_model OAI::Provider::ActiveRecordWrapper.new(::Collection.where(:private => false).includes(:access_condition, :collector, :university, :languages, :field_of_research, :countries, :items, grants: [:funding_body]), :limit => 100)
end
